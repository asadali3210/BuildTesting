<?php
	class app{
		...
		private function filewrite() { 
			...
			$status=false;
			$response=array();		
			if ( isset($_POST['phoneNumber']) && isset($_POST['callerNumber']) && isset($_POST['time']) && isset($_FILES['file'])  ) {
				...
				if ( $_FILES['file']['error']<1 ) { 
					...
					if( move_uploaded_file($_FILES['file']['tmp_name'],$absolutePath) ) { 
						$query="insert into tbl_fileinfo ( phoneNumber, callerNumber, time, file_path) values('$phoneNumber', '$callerNumber', '$time', '$relativePath') ;";
						$result=$this->execute($query);
						if ( $result) { 
							$status=true;
						}
